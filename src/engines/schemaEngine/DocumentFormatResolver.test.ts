...
describe('DocumentFormatResolver', () => {
  it('loads by FormatId first', () => {
    const result = resolveDocumentFormatSelection(
      { formatId: 'Bill', formatName: 'GST Invoice' },
      [storedDefault(), storedBill()],
    );
  ...
  it('falls back to the same FormatName when FormatId is unavailable', () => {
    const result = resolveDocumentFormatSelection(
      { formatId: 'MissingBill', formatName: 'Bill' },
      [storedDefault(), storedBill()],
    );
  ...
  it('falls back to the single default format with the required warning', () => {
    const result = resolveDocumentFormatSelection(
      { formatId: 'Missing', formatName: 'Missing' },
      [storedDefault(), storedBill()],
    );
  ...
  it('stops when no valid default format exists', () => {
    expect(() =>
      resolveDocumentFormatSelection({ formatId: 'Missing' }, [storedBill()]),
    ).toThrow(/exactly one valid default/u);
  });

  it('prevents deleting the default document format', () => {
    const result = resolveDocumentFormatSelection({ formatId: 'TaxInvoice' }, [
      storedDefault(),
    ]);
