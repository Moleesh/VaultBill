...
export const ReportsPage: FC = () => {
  ...
  useEffect(() => {
    if (window.vaultBillDesktop) {
      ...
      void requestHostedApi<{ readonly isExpired: boolean }>('/trial/status').then((trial) => {
        setTrialExpired(trial.isExpired);
      });
    }
  ...
  useEffect(() => {
    ...
    void requestReportPage(query)
      ...
      .catch((reason: unknown) => {
        if (active)
          setPageError(
            reason instanceof Error ? reason.message : 'Report data could not be loaded.',
          );
      })
      .finally(() => {
            ...
            void requestReportPage({ ...query, cursor: nextCursor })
              .then((page) => {
                setServerRecords((current) => [
                  ...
                  ...page.rows.filter(
                    (record) =>
                      !current.some((candidate) => candidate.recordId === record.recordId),
                  ),
                ]);
                setServerTotal(page.total);
                setNextCursor(page.nextCursor);
  ...
  const runNextRecordBatch = (startAt = 0, suppliedRecords?: readonly AppRecord[]) => {
    ...
    void (prepare ? Promise.resolve(prepare) : loadCompleteResult())
      .then((completeRecords) => {
        ...
        if (printable.length === 0) throw new Error('No finalized or cancelled records match.');
    ...
    <div className="page-stack reports-page">
      ...
      <section className="data-panel">
        ...
        <div className="report-toolbar">
          ...
          <div>
            <button disabled={totalRecords === 0 || trialExpired} onClick={exportAll} type="button">
              ...
              <button
                onClick={() => {
                  ...
                  const cancel = window.vaultBillDesktop
                    ...
                    : capabilities.isLanBrowser
                      ? requestHostedApi('/print/cancel', 'POST', { jobId: task.jobId })
...
const buildTaxSummary = (records: readonly AppRecord[]) => {
  const summary = new Map<string, { rate: string; taxable: number; tax: number; count: number }>();
...
const buildCustomerLedger = (records: readonly AppRecord[]) => {
  ...
  for (const record of records) {
    ...
    if (record.status === 'Finalized') current.revenue += Number.parseFloat(record.grandTotal) || 0;
    ledger.set(key, current);
  }
  return [...ledger.values()].sort(
    (left, right) => right.revenue - left.revenue || left.customer.localeCompare(right.customer),
  );
};
