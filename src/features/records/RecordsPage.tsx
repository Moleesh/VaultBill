...
export const RecordsPage: FC = () => {
  ...
  useEffect(() => {
    const inventory = window.vaultBillDesktop
      ...
      : capabilities.isLanBrowser
        ? requestHostedApi<readonly { readonly formatId: string; readonly formatName: string }[]>(
            '/print/formats',
          )
        : undefined;
    void inventory
  ...
  const updateLineItem = (rowId: string, changes: Partial<RecordLineItem>) => {
    ...
    const grandTotal = lineItems
      ...
      .toFixed(2);
    markChanged(
      applyDocumentCalculations({ ...record, lineItems, grandTotal }, activePrintPackage?.config),
    );
  };
    ...
    <div className="page-stack records-page">
      <div className="operational-header">
        <div>
          ...
          <h1>
            {activeTab === 'create' ? `Create ${record.formatName}` : 'Find and reprint records'}
          </h1>
        </div>
        {activeTab === 'create' ? (
          <SearchableDropdown
            ...
            <div className="form-grid">
              ...
              {configuredDocumentFields.map((field) => (
                <ConfiguredField
                  ...
                  value={record.fieldValues?.[field.FieldId] ?? defaultFieldValue(field)}
                />
              ))}
            </div>
                        ...
                        <ConfiguredField
                          ...
                          onChange={(value) => {
                            updateLineItem(item.rowId, {
                              values: { ...(item.values ?? {}), [field.FieldId]: value },
                            });
                          }}
                          value={item.values?.[field.FieldId] ?? defaultFieldValue(field)}
                        />
                      ))}
                    </div>
                  ) : null}
              ...
              <button
                onClick={() => {
                  ...
                  const cancel = window.vaultBillDesktop
                    ...
                    : capabilities.isLanBrowser
                      ? requestHostedApi('/print/cancel', 'POST', { jobId: outputTask.jobId })
  ...
  for (const field of config.Fields.filter(
    ...
    if (sumMatch?.[1]) {
      value = record.lineItems
        .reduce(
          (total, item) =>
            total + (Number.parseFloat(lineItemFieldValue(item, sumMatch[1] ?? '')) || 0),
          0,
        )
    ...
    else next = { ...next, fieldValues: { ...(next.fieldValues ?? {}), [field.FieldId]: value } };
  }
  return next;
};
...
const firstMissingRequiredField = (
  ...
  config.Fields.find(
    (field) =>
      field.Required && !field.Calculated && !documentFieldValue(record, field.FieldId).trim(),
  )?.Label;
