...
export class BackupService {
  ...
  public createArchive = (encryptionEnabled: boolean, password?: string): BackupArchive => {
    ...
    try {
      ...
      const encrypted = encryptionEnabled ? encryptDatabase(databaseBytes, password) : undefined;
      const manifest = {
        Product: 'VaultBill',
...
const encryptBytes = (bytes: Uint8Array, key: Buffer, iv: Buffer): Uint8Array => {
  ...
  return new Uint8Array(Buffer.concat([cipher.update(bytes), cipher.final(), cipher.getAuthTag()]));
};
...
const decryptBytes = (bytes: Uint8Array, key: Buffer, iv: Buffer): Uint8Array => {
  ...
  return new Uint8Array(
    Buffer.concat([decipher.update(payload.subarray(0, payload.length - 16)), decipher.final()]),
  );
};
