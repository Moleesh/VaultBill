  ...
  public save = (rawPackage: unknown): BuilderPackage => {
    ...
    try {
      this.#database
        ...
        .run(formatId, formatName, JSON.stringify(builderPackage.config), hasDefault ? 0 : 1, now);
      this.#database
        .prepare(
...
const sanitizeSvg = (svg: string) => {
  if (
    ...
    throw new Error('SVG assets cannot contain scripts, active content, or external resources.');
  }
};
