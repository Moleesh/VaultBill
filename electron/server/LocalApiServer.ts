...
export class LocalApiServer {
    ...
    async #handle(request: IncomingMessage, response: ServerResponse): Promise<void> {
        ...
        try {
            ...
            if (request.method === 'POST' && request.url === '/backup/restore') {
                ...
                const backupPassword = decodeHeaderSecret(
                    request.headers['x-vaultbill-backup-password'],
                );
            ...
            if (request.method === 'POST' && request.url === '/settings/business') {
                ...
                this.#send(
                    response,
                    200,
                    this.#requireDataOperations().saveBusinessSettings(
            ...
            if (request.method === 'POST' && request.url === '/settings/integrations') {
                ...
                this.#send(
                    response,
                    200,
                    this.#requireDataOperations().saveIntegrationSettings(
                        await this.#readBody(request),
                    ),
                );
                return;
            ...
            if (request.method === 'GET' && request.url === '/builder/inventory') {
                if (account.role !== 'SysAdmin') {
                    throw new ApiError(
                        403,
                        'Only the System Administrator can view Builder inventory.',
                    );
                }
            ...
            if (request.method === 'POST' && request.url === '/builder/package') {
                if (account.role !== 'SysAdmin') {
                    throw new ApiError(
                        403,
                        'Only the System Administrator can publish Builder formats.',
                    );
                }
                if (this.#recordStore.getTrialStatus().isExpired) {
                    throw new ApiError(
                        403,
                        'The trial is read-only. Enter a license key to use Builder.',
                    );
                }
            ...
            if (request.method === 'POST' && request.url === '/print/html') {
                if (this.#recordStore.getTrialStatus().isExpired) {
                    throw new ApiError(
                        403,
                        'The trial is read-only. Enter a license key to print.',
                    );
                }
            ...
            if (request.method === 'POST' && request.url === '/print/cancel') {
                ...
                if (!cancelPrint)
                    throw new ApiError(503, 'Host print cancellation is unavailable.');
            ...
            if (request.method === 'POST' && request.url === '/reports/query') {
                ...
                this.#send(
                    response,
                    200,
                    this.#recordStore.queryReport(await this.#readBody(request)),
                );
                return;
            }
    ...
    #requireCsrf = (request: IncomingMessage, session: HostedSession) => {
        ...
        if (typeof supplied !== 'string' || !safeEqual(supplied, session.csrfToken)) {
            throw new ApiError(
                403,
                'The hosted request could not be verified. Refresh and try again.',
            );
        }
    };
    ...
    #readRawBody = async (request: IncomingMessage): Promise<Uint8Array> => {
        ...
        for await (const chunk of request) {
            ...
            size += buffer.length;
            if (size > MAX_LOCAL_API_BODY_BYTES)
