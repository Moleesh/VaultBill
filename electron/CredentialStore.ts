...
export class CredentialStore {
  ...
  public resetPassword = (userId: string, password: string): DesktopOperatorAccount => {
    ...
    const result = this.#database
      ...
      .run(salt, hash, password === defaultCredential ? 1 : 0, new Date().toISOString(), userId);
  ...
  #bootstrap = () => {
    ...
    const backup = this.#database
      .prepare("SELECT setting_key FROM app_secure_settings WHERE setting_key = 'backup_password';")
      .get();
